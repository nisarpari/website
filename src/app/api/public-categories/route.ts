// GET /api/public-categories - Fetch eCommerce public categories directly
...
export async function GET() {
  try {
    const categories = await getCachedOrFetch<TransformedCategory[]>(
      CACHE_KEYS.PUBLIC_CATEGORIES,
      CACHE_TTL.CATEGORIES,
      async () => {
        const rawCategories = await odooApiCall<OdooPublicCategory[]>(
    ...
    return cachedJsonResponse(categories, HTTP_CACHE_TTL.LONG);
  } catch (error) {
    console.error('Error fetching public categories:', error);
    return new Response(JSON.stringify({ error: 'Failed to fetch public categories' }), {
      status: 500,

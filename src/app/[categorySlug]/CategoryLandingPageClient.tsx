...
interface CategoryLandingPageClientProps {
  categoryId: number;
}

export default function CategoryLandingPageClient({ categoryId }: CategoryLandingPageClientProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (cat && cat.childIds && cat.childIds.length > 0) {
        ...
        const childProductPromises = cat.childIds.map(async (childId: number) => {
          try {
  ...
  const childCategories = categories.filter((c: Category) => category.childIds?.includes(c.id));
